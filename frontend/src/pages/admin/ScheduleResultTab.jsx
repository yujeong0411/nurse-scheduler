...
// 같은 주 범위 반환 (1-based day 기준, 최대 NUM_DAYS까지)
function weekRange(day) {
  const w = Math.floor((day - 1) / 7)
  return { start: w * 7 + 1, end: Math.min(w * 7 + 7, NUM_DAYS) }
}

// OFF 신청 전용: 주당 제약·공휴일 등 개인 하드 이유 감지
function inferOffPersonalReasons(day, requestedCode, assignedShift, nurseId, scheduleData, rules) {
  ...
  // 1. 공휴일 제한 — 이날이 공휴일이면 법휴만 가능
  if (holidays.has(day)) {
    reasons.push('공휴일에는 법휴만 배정 가능합니다 (OFF 배정 불가).')
    return reasons
  }

  // 2. 같은 주 이미 _OFF 배정 (H11: 주당 1개)
  for (let d = start; d <= end; d++) {
    if (d === day) continue
    if (get(d) === 'OFF') {
      reasons.push(`이번 주 ${d}일에 OFF가 배정되어 주당 1개 제약으로 추가 배정이 불가합니다.`)
      return reasons
    }
  }

  // 3. 같은 주 법휴로 OFF 면제 (H11: 법휴가 대체)
  for (let d = start; d <= end; d++) {
    if (holidays.has(d) && get(d) === '법휴') {
      reasons.push(`이번 주 ${d}일(공휴일) 법휴가 주휴 역할을 대체해 OFF 추가 배정이 불가합니다.`)
      return reasons
    }
  }

  // 4. 다른 타입 배정 (수면·생휴 등)
  if (assignedShift && assignedShift !== 'OFF' && !WORK_SET.has(assignedShift)) {
    reasons.push(`이날 ${assignedShift}(으)로 배정되어 OFF 배정이 불가했습니다.`)
  }
...
function inferFailureReasons(day, requestedCode, assignedShift, nurseId, scheduleData, nurses) {
  ...
  // ── 연속 근무 한도 (근무 신청이 거절된 경우에만 표시) ──
  // 휴무/OFF 신청 거절 사유로는 연속 근무 한도가 무관하므로 제외
  if (WORK_SET.has(requestedCode)) {
    let consecBefore = 0
    for (let d = day - 1; d >= Math.max(1, day - 5); d--) {
      if (WORK_SET.has(get(nurseId, d))) consecBefore++
      else break
    }
    let consecAfter = 0
    for (let d = day + 1; d <= Math.min(31, day + 5); d++) {
      if (WORK_SET.has(get(nurseId, d))) consecAfter++
      else break
    }
    const totalConsec = consecBefore + 1 + consecAfter
    const runStart = day - consecBefore
    if (totalConsec >= 5) {
      reasons.push(`${runStart}일부터 ${totalConsec}일 연속 근무입니다. (최대 연속 근무 5일 한도)`)
    }
  }
        ...
        >
          ...
          <div className="overflow-y-auto" style={{ maxHeight: 240 }}>
            {logPopup.entries === null ? (
              ...
              // ── 경쟁자 있는 경우: 개인 하드 이유 + 신청자 순위 ──
              <div>
                {logPopup.unmatched && logPopup.nurseId && (() => {
                  const personalReasons = inferOffPersonalReasons(
                    logPopup.day, logPopup.code, logPopup.assignedShift,
                    logPopup.nurseId, scheduleData, rulesData
                  )
                  if (!personalReasons.length) return null
                  return (
                    <div className="mx-3 mt-2 mb-1 rounded-lg bg-amber-50 border border-amber-200 px-2.5 py-2 space-y-1">
                      <p className="text-[10px] font-bold text-amber-700">배정 불가 이유 (솔버 제약)</p>
                ...
                <p className="px-3 pt-1.5 pb-0.5 text-[10px] font-semibold text-slate-400">신청 경쟁 현황 (참고용)</p>
