...
### 1. 설정 탭
간호사 목록 등록 (이름, 역할, 직급, 임산부/남자/주4일제, 고정주휴, 휴가잔여 등). 규칙 엑셀 / 신청표 엑셀 / 이전 근무표 불러오기 지원.

![설정 탭](docs/settings.png)

### 2. 요청사항 탭
개인별 근무 희망(D/E/N), 휴무 요청(OFF/법휴/휴가 등), 제외 요청(D 제외/E 제외/N 제외)을 날짜별 콤보박스로 입력.

![요청사항 탭](docs/requests.png)

### 3. 규칙설정 탭
일일 최소 인원, 근무 순서/연속 제한, 직급 제약, 특수 조건(임산부/생리휴무), 수면 휴무 발생 조건 등 설정.

![규칙설정 탭](docs/rule_settings.png)

### 4. 결과 탭
근무표 자동 생성 → 색상 코딩된 결과 확인 → 셀 단위 수동 수정(위반 체크) → 엑셀 내보내기. 통계(D/E/N 편차, 요청 반영률, 역순 패턴)도 함께 표시.
